...
const DURATION_SEC = parseInt(__ENV.DURATION_SEC || "240", 10);   // ✅ 4 minutes = 240s
...
export default function () {
    ...
    // small jitter to avoid perfect sync burst
    ...
    while (Date.now() < endMs) {
        ...
        // 2) before 20s -> keep traffic with 5-char texts
        else if (elapsedSec < 20) {
            sentText.add(1);
            postEvent(
                makeEnvelope(userId, {
                    message: { mid, text: randString(5) },
                })
            );
        }
        // 3) at ~20s -> first postback
        else if (!didFirstPostback) {
            sentPostback.add(1);
            postEvent(
                makeEnvelope(userId, {
                    postback: { mid, payload: pickPostback() },
                })
            );
            didFirstPostback = true;
        }
        // 4) 20s..80s -> 5-char texts
        ...
        // 5) after 80s -> postbacks till end
        else {
            sentPostback.add(1);
            postEvent(
                makeEnvelope(userId, {
        ...
        // pace to ~200 rps total
        const spentSec = (Date.now() - loopStart) / 1000;
        sleep(Math.max(0, PER_USER_INTERVAL_SEC - spentSec));
    }
}
